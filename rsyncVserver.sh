...
sh /ruta/de/Vserver/vServer.sh -s /port=690 2> /dev/null # Aquí indicar la ruta del script vServer.sh
...
if [ "$(pgrep vServer)" ] && [ "$day_num" -ne "6" ] && [ "$day_num" -ne "7" ] ;then
    sh /ruta/de/Vserver/vServer.sh -t /port=690 &> /dev/null # Aquí indicar la ruta del script vServer.sh
    rsync -az "${src_dir}" "${host_dest}":"${dest_dir}" 
    sh /ruta/de/Vserver/vServer.sh -s /port=690 &> /dev/null			
fi
exit 0
